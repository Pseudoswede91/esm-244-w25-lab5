...
```{r}
seeds_df <- read_tsv(here::here('data','seeds_dataset.txt')) #note that this is read_tsv, which is reading a "tab separated values"
#here::here is a "lazy load". instead of installing the whole package and using more memory on your computer, you can dip in and use just the "here" function.
...
A: There are extremely low values that are essentially "na's". the original data scientist didn't have an NA in their system, so they had to put something in for the values. We need to find a way to transform the -999's into NA values.
The variety is 1, which is a numerical value. it needs to be a factor.
...
seeds_df <- read_tsv(here::here('data','seeds_dataset.txt'),
                     col_names = FALSE,
                     na = '-999') %>% #using read_whatever commands, you can put NA's in!
  setNames(var_names) %>%
  mutate(variety = case_when(variety == 1 ~ 'Kama', #"in the column variety, if the value is exactly 1, change it to Kama
                             variety == 2 ~ 'Rosa',# the same but Rosa
                             variety == 3 ~ 'Canadian', #the same but Canada
                             TRUE ~ 'oops')) #if it's anyhting else, put "oops"

# A Case_When Function allows the above to happen.
...
# try other variations: 3) A scatter plot with length of kernel groove on the x-axis and width of kernel on y-axis.
...
```{r}
#| fig-cap: We would select 3 groups to initiate a kmean cluster because the kink in the graph is most pronounced at t3 clusters
...
```{r}
#| tbl-cap: consistency table for the grouping of kmeans and varietals of kernels
...
seeds_cl %>% 
  ...
  table() %>% 
#how can we make this table look beautiful in quarto? Can allso do the table-cap up above
  knitr::kable()
```
...
Write pseudocode for what we will need to do for heirarchal clustering:
1) slice the top 20 emitters

2) drap nas (note that we don't have any)
...
7) think about what linkages we want (either single or complete)
...
wb_scaled <- wb_ghg_20 %>% 
  select(3:7) %>% #this is hard coding, not relative coding. relative would be (-c(name,region))
...
p_complete+p_single
```
In Single, Sudan and CAR are off on their own. In complete, they're mixed in. In complete, the US and China are mixed in together
The interpretation is more difficult than running the clustering. 
...
#mutate and add vector on to our original dataframe
wb_ghg_20 <- wb_ghg_20 %>% 
  mutate(cluster = hc_cut)



#make a plot with these groupings
